...
└── lib/
    └── elixir_scope/  # Or specific subdirectory like 'ast_repository'
                ElixirScope.Capture.PipelineManager;elixir_scope_utils;elixir_scope_config;elixir_scope_events;elixir_scope_capture_core;capture_pipeline/
        └── lib/
            └── elixir_scope/
                └── capture/
                    ├── pipeline/
...
*   [**EXPAND HERE: List 3-5 key public functions or GenServer calls. Example for `elixir_scope_ast_repo`:**]
    *   `ElixirScope.Capture.Pipeline.Repository.get_cpg(module_name, function_name, arity)`
    *   `ElixirScope.Capture.Pipeline.Repository.query_static_analysis(query_spec)`
    *   `ElixirScope.Capture.Pipeline.CPGMath.calculate_centrality(cpg_data, node_id, type)`
    *   `ElixirScope.Capture.Pipeline.CPGSemantics.detect_architectural_smells(cpg_data, opts)`
    *   `ElixirScope.Capture.Pipeline.PatternMatcher.match_cpg_pattern(cpg_data, pattern_definition)`
...
Within the `TidewaveScope` ecosystem, the `elixir_scope_capture_pipeline` library will [**EXPAND HERE: Describe how `TidewaveScope`'s plug, MCP tools, or other ElixirScope libraries will interact with this one. Example for `elixir_scope_ast_repo`:**]
